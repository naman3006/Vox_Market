...
export class WishlistService {
  ...
  async findByToken(token: string): Promise<WishlistDocument> {
    const wishlist = await this.wishlistModel
      .findOne({ shareToken: token, privacy: 'public' })
      .populate('items.productId')
      .exec();
    if (!wishlist)
      throw new NotFoundException('Wishlist not found or is private');
    return wishlist;
  }
  ...
  async add(
    ...
    console.log(
      `[WishlistService.add] Request - User: ${userId}, Product: ${productId}, TargetList: ${wishlistId}`,
    );
    let targetId = wishlistId;

    if (!targetId) {
      // Find default or first wishlist
      let def: WishlistDocument | null = await this.wishlistModel
    ...
    if (!updatedWishlist) {
      console.error(
        `[WishlistService.add] Wishlist not found for ID: ${targetId}`,
      );
  ...
  async remove(
    ...
    const filter: FilterQuery<WishlistDocument> = {
      userId: new Types.ObjectId(userId),
    };
  ...
  async toggleBoughtStatus(
    shareToken: string,
    productId: string,
    boughtBy: string,
  ): Promise<WishlistDocument> {
    const wishlist = await this.wishlistModel
      .findOne({ shareToken, privacy: 'public' })
      .exec();
    if (!wishlist)
